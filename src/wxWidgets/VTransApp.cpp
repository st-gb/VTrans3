...
DEFINE_LOCAL_EVENT_TYPE(LoadingDictFileFailedEvent)
...
EVT_COMMAND(wxID_ANY, LoadingDictFileFailedEvent, VTransApp::OnLoadingDictFileFailed)
END_EVENT_TABLE()
...
void VTransApp::LoadingVocabularyFileFailed(
  ...
  wxCommandEvent wxcommand_event(LoadingDictFileFailedEvent);
  AddPendingEvent(wxcommand_event);
...
void VTransApp::OnLoadingDictFileFailed(wxCommandEvent & event) {
  /** Disable all actions because the loading the file failed, i.e. the 
     file handle may be invalid and so this program could crash else. */
  DisableDictAccessingActions();
}
...
void VTransApp::SetDictionaryFilePath(const wxString & wxstrDictionaryFilePath) {
  m_p_mainWindow->SetDictionaryFilePath(wxstrDictionaryFilePath);
  m_stdstrVocabularyFilePath = wxWidgets::GetStdString(wxstrDictionaryFilePath);
}
